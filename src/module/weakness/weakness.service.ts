...
export class WeaknessService {
  ...
  async getGrammarWeakness(idUser: string) {
    ...
    for (const v of violations) {
      if (!grouped[v.idGrammar]) {
        grouped[v.idGrammar] = { idGrammar: v.idGrammar, title: v.grammar.title, wrongCount: 0 };
      }
  ...
  async getQuestionTypeWeakness(idUser: string) {
    const performances = await this.databaseService.questionTypePerformance.findMany({
      where: {
        idUser,
        totalAttempts: { gte: 3 },
        errorRate: { gte: 0.3 }
